...
describe("macOS bundle naming", () => {
  ...
  it("keeps CFBundleName aligned with productName for Electron helpers", () => {
    assert.equal(pkg.build.productName, "kirocrew-customapi");
    assert.equal(
  ...
  it("uses CFBundleDisplayName for spaced stable and nightly names", () => {
    assert.equal(extendInfo.CFBundleDisplayName, "kirocrew-customapi");
    assert.match(
      buildScript,
...
describe("first-download installer design contract", () => {
  ...
  it("shows native progress for updates without adding setup decisions", () => {
    ...
    assert.match(
      installer,
      /!macro customInstallMode[\s\S]*?\$KiroVisibleUpdate == 1[\s\S]*?\$KiroHasPerMachineInstallation == 1[\s\S]*?\$KiroHasPerUserInstallation == 0[\s\S]*?StrCpy \$isForceMachineInstall 1[\s\S]*?\$KiroHasPerUserInstallation == 1[\s\S]*?\$KiroHasPerMachineInstallation == 0[\s\S]*?StrCpy \$isForceCurrentInstall 1/
    );
    assert.match(
      installer,
    ...
    assert.match(
      installer,
      /Function KiroInstFilesShow[\s\S]*?\$KiroVisibleUpdate == 1[\s\S]*?StrCpy \$KiroUpdateCommitted 1[\s\S]*?GetDlgItem \$0 \$HWNDPARENT 1038[\s\S]*?SendMessage \$0 \$\{KIRO_WM_SETTEXT\} 0 "STR:\$\(KiroUpdateProgress\)"[\s\S]*?EnableWindow \$0 0[\s\S]*?ShowWindow \$0 \$\{KIRO_SW_HIDE\}[\s\S]*?FunctionEnd/
    );
    assert.match(
      installer,
      /Function KiroAbortPre[\s\S]*?\$KiroVisibleUpdate == 1[\s\S]*?\$KiroUpdateCommitted == 1[\s\S]*?Abort[\s\S]*?FunctionEnd[\s\S]*?!define MUI_PAGE_FUNCTION_ABORTWARNING KiroAbortPre/
    );
