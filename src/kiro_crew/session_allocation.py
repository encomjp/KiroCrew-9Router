...
def _model_is_text_only_for_session(cfg: "KiroCrewConfig", model: str | None) -> bool:
    """Fork: True when *model* cannot take image content for the session's routing.
    ...
    prompt path so a registry-known vision model resumes normally while the
    router text-only denylist stays protected.
    """
    if not model:
        return False
    try:
...
class SessionRegistryState:
    ...
    touch_lock: threading.Lock = field(default_factory=threading.Lock)
...
class SessionAllocationService:
    ...
    def touch(self, key: str) -> bool:
        """Fork: mark *key* as recently used so the idle sweep does not reap it.
        ...
        across a long turn ages as if it were idle. Thread-safe via
        ``_touch_lock`` so a concurrent ``wait`` keepalive and the periodic
        idle sweep do not race ``last_used`` (H7 bounds).
        """
        session = self._sessions.get(self._owner._fold_key(key))
        if session is None:
            return False
        with self.state.touch_lock:
            session.last_used = time.monotonic()
        return True
    ...
    async def get_or_create(
        ...
        **extra_factory_kwargs: Any,
        ...
        # Fork: text-only image guard — a session resuming on a text-only router
        # model would replay any image blocks stored in its history (from before
        # a model switch or a previous vision-model session), and the text-only
        # upstream rejects them (400). Discard the resume so the session starts
        # fresh on the text-only model.
        if resume_sid and model and _model_is_text_only_for_session(owner._cfg, model):
            logger.info(
                "Discarding resume for %s: model is text-only (image history "
