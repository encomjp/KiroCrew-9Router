...
class KiroCrewConfig:
    ...
    def create_provider_factory(self) -> Callable:
        ...
        # Fork: claude_code / opencode backend selection. "acp" (default) drives
        # kiro-cli; "claude_code" drives claude-agent-acp through the dormant
        # ACP_BACKEND_CLAUDE seam; "opencode" drives the fork's OpenCode adapter.
        ...
        def _acp(
            ...
            **_kwargs: object,
            ...
            if (
                ...
                m = self.agent.model
            # Translation boundary fork delta: on the claude_code path the
            # canonical keys ARE the wire format, so translate to the
            # claude_code provider id. With a custom base URL (9router,
            # CLIProxyAPI) the id is the router's own namespace — never
            # registry-translate.
            if provider_backend == "claude" and not provider_base_url:
            ...
            if (
                ...
                m = self.agent.model
            # Translation boundary — acp_effective_model already applied
            # to_acp_id (canonical key -> kiro id). Fork deltas on top:
            # on the claude_code path the canonical keys ARE the wire
            ...
            # Fork: agent.provider=claude_code|opencode drives the provider backend
            # directly (the ACP adapter's own claude/opencode branch), overriding
            # the member/acp_backend selection for this session.
            if provider_backend:
