...
- **Windows updates are visible and normally non-interactive.** `quitAndInstall` passes
  ...
  Welcome, unambiguous install-mode, and Finish decisions, leaving only the
  native extraction page and its real progress visible. If Windows has both a
  registered per-user and per-machine copy, electron-updater does not identify
  which copy launched it, so the native scope chooser remains visible instead
  of silently updating the wrong copy; Cancel remains available on that chooser
  until file replacement begins. At completion it runs the locked
  electron-builder `StartApp` contract and exits successfully. The same
