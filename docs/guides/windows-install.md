...
- **Auto-updates stay visible without normally becoming interactive** — after
  the app stops its local gateway and closes, the update path skips Welcome,
  an unambiguous install scope, and Finish, but leaves the native NSIS
  extraction page on screen with real progress. If both a per-user and a
  per-machine copy are registered, the native scope chooser stays visible so
  the updater cannot silently target the wrong copy, and it can still be
  canceled until replacement starts. At 100% it reopens Kiro Crew and closes
  automatically. A
